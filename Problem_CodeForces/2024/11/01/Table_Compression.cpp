...
void solve()
{
    ...
    sort(all1(pl), [&](const node &x, const node &y)
    ...
    for (int i = 1; i <= n * m; i++)
    {
        int fx = dsu.find(px[pl[i].x]);
        int fy = dsu.find(py[pl[i].y]);
        int p = dsu.find(pl[i].pos);
